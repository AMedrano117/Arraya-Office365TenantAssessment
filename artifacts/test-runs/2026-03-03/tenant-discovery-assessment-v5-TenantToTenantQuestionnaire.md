...
| Email domains to migrate | 4 custom domain(s): jeffersondemo.cavehousebrewing.com, cavehousebrewing.com, lab.com, lvhndemo.cavehousebrewing.com |
...
- Primary migration review items: Guests and external identities (Review); Teams workload data (Needs Data); Voice workload readiness (Needs Data); Verified custom domains (Blocker); Mail flow dependencies (Review); Mail routing (Review).
- Current assessment findings to review: License 'SPE_E5' has 0 remaining (fully allocated) 108 SharePoint sites exceed 1024 GB B2B management policy not found; external identity settings may be default/unconfigured License 'POWERAPPS_PER_USER' has 0 remaining (fully allocated)
