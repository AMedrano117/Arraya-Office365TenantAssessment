...
- Primary migration review items: Guests and external identities (Review); Mail routing (Review); Public folders (Review); Voice workload readiness (Needs Data); Teams workload data (Needs Data); Mail flow dependencies (Review).
- Current assessment findings to review: Microsoft Secure Score is 39.8%, which is below the target range for a mature tenant baseline. Device compliance is only 0% (target: 80%) License 'Office 365 E3' has 0 remaining (fully allocated) 5 OneDrive sites exceed 1024 GB
