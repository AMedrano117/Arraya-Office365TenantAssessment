...
| Email domains to migrate | 4 custom domain(s): cavehousebrewing.com, lab.com, lvhndemo.cavehousebrewing.com, jeffersondemo.cavehousebrewing.com |
...
- Automated tenant discovery generated on 2026-03-04 for Cavehouse Brewery.
- Primary migration review items: Mail flow dependencies (Review); Mail routing (Review); Directory synchronization (Review); Public folders (Review); Voice workload readiness (Needs Data); Verified custom domains (Blocker).
- Current assessment findings to review: License 'MCOPSTN1' has 0 remaining (fully allocated) 108 SharePoint sites exceed 1024 GB B2B management policy not found; external identity settings may be default/unconfigured Microsoft Secure Score is 39.8%, which is below the target range for a mature tenant baseline.
