Describe 'Get-FullTenantReportDetails permission preflight' {
    ...
    It 'defines a permission preflight with an explicit skip switch and invokes it before collection starts by default' {
        ...
        $script:collectorSource | Should -Match '\$runUseExistingConnections = \$UseExistingConnections\.IsPresent'
        $script:collectorSource | Should -Match 'UseExistingConnections can only be used with live data collection'
        $script:collectorSource | Should -Match 'Test-AssessmentExistingSessions -WorkloadPlan \$assessmentAuthWorkloadPlan -TenantId \$TenantId -ModeLabel ''UseExistingConnections'''
        $script:collectorSource | Should -Match 'Using existing Microsoft 365 connections for data collection\. Authentication and permission preflight are skipped\.'
        $script:collectorSource | Should -Match 'Ensure-AssessmentServiceContext .* -ExistingConnectionsOnly:\$runUseExistingConnections'
        $script:collectorSource | Should -Match 'Existing connections ready for data collection\.'
        $script:collectorSource | Should -Match 'Run Invoke-M365TenantConnectionPreflight first or omit -UseExistingConnections'
        $script:collectorSource | Should -Match 'Permission preflight skipped'
    ...
    It 'uses an assessment-owned auth orchestrator instead of the generic Office365 connector bootstrap' {
        ...
        $script:collectorSource | Should -Match 'Auth mode'
    ...
    It 'connects Exchange Online before Microsoft Graph for interactive assessment runs' {
        ...
        $script:collectorSource | Should -Match 'preflight \(existing sessions may be reused\)'
    ...
    It 'makes the broader interactive connection path operator-friendly across Exchange, SharePoint, Teams, and Purview' {
        ...
        $script:collectorSource | Should -Match 'SharePoint admin\.\.\.'
        ...
        $script:collectorSource | Should -Match "Write-WorkloadStatus -Label 'Purview'"
    ...
    It 'uses a stable preflight progress counter and suppresses inner Graph record-count progress' {
        $script:collectorSource | Should -Match 'access verified.*\{1\} checks'
